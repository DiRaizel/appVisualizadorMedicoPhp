...
class Visualizador extends CI_Model {
    ...
    function traerIpVisualizador() {
        ...
        //
        $query = $this->db->query("select IpVisualizador from configtv where estado = $visualizador");
        //
        $datos = array();
        //
    ...
    function cargarListaVisualizadores() {
        //
        $query = $this->db->query("select estado from configtv");
        //
        $datos = array();
        //
        if (count($query->result()) > 0) {
    ...
    function cargarTurno() {
        ...
        $query = $this->db->query("select IdTv, NombrePaciente, Modulo, "
                ...
                . "(select MIN(IdTv) from tv WHERE Estado = $visualizador)");
        //
        $datos = array();
        //
        if (count($query->result()) > 0) {
    ...
    function cargarVideos() {
        ...
        $query = $this->db->query("select idvideo, nombrevideo, volumen from "
                . "videos where estado = $visualizador");
        //
        $datos = array();
        //
        if (count($query->result()) > 0) {
